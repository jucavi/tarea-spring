...
import com.example.tareaspring.csvmodels.SigningCSV;
    ...
    */
    ...
    public List<Signing> parseCSVFileToSignings(@NonNull MultipartFile file) {
        ...
        try (Reader reader = new BufferedReader(new InputStreamReader(file.getInputStream()))) {
            ...
            List<SigningCSV> SigningsObj = csvToBean.parse();

            for (SigningCSV p : SigningsObj) {
                Signing Signing = p.toBeanWithId();
                signings.add(Signing);
            }

            repository.saveAll(signings);
